...
class ParametrizedSpiralingDistribution:
  ...
  def generate(self, particle_radius: float, center: np.ndarray, radius: float) -> np.ndarray:
    ...
    # Adjust the last shell to ensure total count matches n
    shell_counts[-1] += n - np.sum(shell_counts)
...
def generate_box(box_size, box_center, r, lattice):
  if lattice == "hcp":
    ...
    df = ParametrizedSpiralingDistribution(seed=0).generate(r, np.array([0.0,0.0,0.0]), max(box_size)/2.0)
...
def rand_rotate(xyz, center):
  ...
  # xyz[["x","y","z"]] = xyz[["x", "y", "z"]].to_numpy() @ rot.T
  xyz["x"] += center[0]
  xyz["y"] += center[1]
  xyz["z"] += center[2]
  return xyz
...
class part():
  ...
  def export(self, path, mode, columns=None, start_idx=0):
    ...
    if mode == "w":
      self.points.to_csv(path, mode=mode, columns=columns, float_format='%.16e')
    if mode == "a":
      self.points.to_csv(path, mode=mode, columns=columns, float_format='%.16e', header=False)
...
'''
Modify initial pressure & density due to self-gravity: assuming sphere
'''
def init_pressure(tar, tar_center, tar_radius, tar_rho, bulk):
  kPI = np.pi
  kG = 6.67430e-11  # gravitational constant
  kEps = 1e-12

  # Sphere gravity center and properties
  center = tar_center
  radius2 = tar_radius ** 2

  # Compute dr vectors and norms
  dr_array = tar.points[['X', 'Y', 'Z']].to_numpy() - center  # shape (N,3)
  dr_norm2 = np.sum(dr_array**2, axis=1)
  dr_norm = np.sqrt(dr_norm2)

  # Compute sigma_r
  sigma_r = -2.0 / 3.0 * kPI * kG * tar_rho**2 * (radius2 - dr_norm2)
  tar.points['P'] = -sigma_r

  # TODO: rotation correction

  # Update rho and mass based on bulk modulus
  tar.points['RHO'] *= 1.0 + tar.points['P'] / bulk
  tar.points['MASS'] *= 1.0 + tar.points['P'] / bulk
